-- Migration: Schema Optimization for AI Scheduling
-- Date: 2025-06-25
-- Description: Adds missing foreign keys, indexes, and helper functions for AI scheduling
...
-- Add school_id to scheduled_lessons if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE "public"."scheduled_lessons" ADD COLUMN "school_id" "uuid";
        ...
        WHERE "public"."scheduled_lessons"."school_id" IS NULL;
        
        -- Make school_id NOT NULL after update
        ALTER TABLE "public"."scheduled_lessons" ALTER COLUMN "school_id" SET NOT NULL;
    END IF;
END $$;
...
DO $$
BEGIN
    -- Add timeslot_id foreign key if it doesn't exist
    IF NOT EXISTS (
    ...
    -- Add teaching_assignment_id foreign key if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- 2. ADD PERFORMANCE INDEXES (with error handling)
-- =====================================================
...
-- Clean up duplicate scheduled lessons before adding unique constraint
-- Remove duplicates, keeping only the first occurrence
DELETE FROM "public"."scheduled_lessons" 
WHERE id IN (
    SELECT id FROM (
        SELECT id,
               ROW_NUMBER() OVER (
                   PARTITION BY teaching_assignment_id, date, timeslot_id 
                   ORDER BY id ASC
               ) as rn
...
-- Add unique constraint to prevent double-booking (if it doesn't exist)
...
-- 4. ADD SIMPLIFIED AI SCHEDULING FUNCTIONS
-- =====================================================

-- Function to get teacher's scheduled times for a date
...
-- Function to get teacher's time constraints
...
-- Function to get class offerings for a term
...
-- Function to get scheduled periods count for an offering
...
-- Function to validate a single scheduled lesson
...
-- Add progress tracking columns to timetable_generations (if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'timetable_generations' 
        AND column_name = 'scheduled_lessons'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'timetable_generations' 
        AND column_name = 'total_offerings'
    ) THEN
        ALTER TABLE "public"."timetable_generations" 
...
-- 6. ADD RLS POLICIES (with error handling)
-- =====================================================

-- Enable RLS on scheduled_lessons if not already enabled
