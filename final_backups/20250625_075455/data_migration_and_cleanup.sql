...
BEGIN
    ...
    LOOP
        ...
        -- Check duration
        IF EXTRACT(EPOCH FROM (slot_record.end_time::time - slot_record.start_time::time)) / 60 < 15 OR
           EXTRACT(EPOCH FROM (slot_record.end_time::time - slot_record.start_time::time)) / 60 > 240 THEN
            PERFORM log_migration_issue(
                'time_slots',
                'invalid_duration',
                ...
                slot_record.id,
                'duration',
                (EXTRACT(EPOCH FROM (slot_record.end_time::time - slot_record.start_time::time)) / 60)::TEXT || ' minutes',
...
BEGIN
    FOR holiday_record IN 
        SELECT h.id, h.date, h.reason, h.term_id, t.start_date, t.end_date
        FROM holidays h
...
WHERE date < (
    SELECT start_date FROM terms WHERE id = holidays.term_id
) OR date > (
    SELECT end_date FROM terms WHERE id = holidays.term_id
);
...
-- Update timestamps
UPDATE schools SET updated_at = NOW() WHERE updated_at IS NULL;
...
SELECT 
    ...
    suggested_fix
FROM migration_issues
ORDER BY severity DESC, table_name, issue_type;
...
BEGIN
    ...
    WHERE EXISTS (
        SELECT 1 FROM migration_issues WHERE severity = 'error'
    );
