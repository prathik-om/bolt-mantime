-- Fixed test script for validation functions
-- This addresses the schema issues found in the previous tests
...
-- 3. Create test terms (15 weeks per term)
INSERT INTO terms (id, academic_year_id, name, start_date, end_date, period_duration_minutes)
VALUES 
  ('33333333-3333-3333-3333-333333333333', '22222222-2222-2222-2222-222222222222', 'Fall Term', '2024-09-01', '2024-12-15', 45),
  ('44444444-4444-4444-4444-444444444444', '22222222-2222-2222-2222-222222222222', 'Spring Term', '2025-01-06', '2025-04-20', 45)
ON CONFLICT (id) DO NOTHING;
...
-- 5. Create test courses (120 hours/year)
...
-- 8. Create test time slots (10 per week)
...
VALUES 
  ...
  ('99998888-7777-6666-5555-444433332222', '11111111-1111-1111-1111-111111111111', 2, '09:40:00', '10:25:00', 3, true, 'Period 3'),
  ('88887777-6666-5555-4444-333322221111', '11111111-1111-1111-1111-111111111111', 3, '08:00:00', '08:45:00', 1, true, 'Period 1'),
  ('77776666-5555-4444-3333-222211110000', '11111111-1111-1111-1111-111111111111', 3, '08:50:00', '09:35:00', 2, true, 'Period 2'),
  ('66665555-4444-3333-2222-11110000ffff', '11111111-1111-1111-1111-111111111111', 3, '09:40:00', '10:25:00', 3, true, 'Period 3'),
  ('55554444-3333-2222-1111-0000ffffeeee', '11111111-1111-1111-1111-111111111111', 4, '08:00:00', '08:45:00', 1, true, 'Period 1')
ON CONFLICT (id) DO NOTHING;

-- 9. Create test class offerings (periods_per_week >= 8 for 120 hours/year over 15 weeks)
INSERT INTO class_offerings (id, term_id, class_id, course_id, periods_per_week, required_hours_per_term)
VALUES 
  ('aaaa1111-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '33333333-3333-3333-3333-333333333333', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '77777777-7777-7777-7777-777777777777', 8, 120),
  ('bbbb2222-bbbb-bbbb-bbbb-bbbbbbbbbbbb', '33333333-3333-3333-3333-333333333333', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '99999999-9999-9999-9999-999999999999', 8, 120),
  ('cccc3333-cccc-cccc-cccc-cccccccccccc', '33333333-3333-3333-3333-333333333333', 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', '88888888-8888-8888-8888-888888888888', 8, 120)
ON CONFLICT (id) DO NOTHING;

-- 10. Create test teaching assignments (all FK references valid)
...
-- 11. Create test holidays (include school_id)
INSERT INTO holidays (id, academic_year_id, date, reason, school_id)
VALUES 
  ('ffff6666-ffff-ffff-ffff-ffffffffffff', '22222222-2222-2222-2222-222222222222', '2024-11-28', 'Thanksgiving', '11111111-1111-1111-1111-111111111111'),
  ('abcd1234-abcd-abcd-abcd-abcdabcdabcd', '22222222-2222-2222-2222-222222222222', '2024-12-25', 'Christmas', '11111111-1111-1111-1111-111111111111')
ON CONFLICT (id) DO NOTHING;
...
BEGIN
  ...
  INSERT INTO class_offerings (id, term_id, class_id, course_id, periods_per_week, required_hours_per_term)
  VALUES 
    ('22223333-4444-5555-6666-777788889999', '33333333-3333-3333-3333-333333333333', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '88888888-8888-8888-8888-888888888888', 35, 525);
  INSERT INTO teaching_assignments (id, school_id, class_offering_id, teacher_id, assignment_type)
...
-- TEST 3: Validate Class Offering Requirements (should fail for insufficient periods)
DO $$
BEGIN
  RAISE NOTICE 'TEST 3: Testing class offering requirements validation (should fail for insufficient periods)...';
  BEGIN
  ...
  END;
  -- Restore valid value
  UPDATE class_offerings SET periods_per_week = 8 WHERE id = 'aaaa1111-aaaa-aaaa-aaaa-aaaaaaaaaaaa';
END $$;
...
BEGIN
  ...
  BEGIN
    INSERT INTO holidays (id, academic_year_id, date, reason, school_id)
    VALUES ('77778888-9999-aaaa-bbbb-ccccddddeeee', '22222222-2222-2222-2222-222222222222', '2023-01-01', 'Invalid Holiday', '11111111-1111-1111-1111-111111111111');
...
BEGIN
  ...
  BEGIN
    INSERT INTO terms (id, academic_year_id, name, start_date, end_date, period_duration_minutes)
    VALUES ('88889999-aaaa-bbbb-cccc-ddddeeeeffff', '22222222-2222-2222-2222-222222222222', 'Fall Term', '2024-09-01', '2024-12-15', 45);
...
BEGIN
  ...
  BEGIN
    INSERT INTO class_offerings (id, term_id, class_id, course_id, periods_per_week, required_hours_per_term)
    VALUES ('9999aaaa-bbbb-cccc-dddd-eeeeffff0000', '33333333-3333-3333-3333-333333333333', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '77777777-7777-7777-7777-777777777777', 8, 120);
...
BEGIN
  ...
  DELETE FROM time_slots WHERE id IN (
    ...
    '88887777-6666-5555-4444-333322221111',
    '77776666-5555-4444-3333-222211110000',
    '66665555-4444-3333-2222-11110000ffff',
    '55554444-3333-2222-1111-0000ffffeeee',
